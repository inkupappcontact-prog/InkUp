-- Création des tables pour les données de l'application
...
-- Ajouter la colonne status à la table purchases si elle n'existe pas
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE public.purchases ADD COLUMN status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'processing', 'shipped', 'delivered'));
    END IF;
END $$;

-- Ajouter la colonne format à la table purchases si elle n'existe pas
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        ALTER TABLE public.purchases ADD COLUMN format TEXT NOT NULL DEFAULT 'digital' CHECK (format IN ('digital', 'physical'));
    END IF;
END $$;

-- Ajouter la colonne shipping_address à la table purchases si elle n'existe pas
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        ALTER TABLE public.purchases ADD COLUMN shipping_address JSONB;
    END IF;
END $$;

-- Ajouter la colonne tracking_number à la table purchases si elle n'existe pas
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- Ajouter la colonne updated_at à la table purchases si elle n'existe pas
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
DROP TRIGGER IF EXISTS update_author_stats_updated_at ON public.author_stats;
CREATE TRIGGER update_author_stats_updated_at BEFORE UPDATE ON public.author_stats
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
