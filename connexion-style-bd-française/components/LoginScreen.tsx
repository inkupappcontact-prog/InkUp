    ...
    <div className="min-h-screen w-full flex flex-col lg:flex-row bg-white overflow-hidden">
      ...
      <div className="lg:w-1/2 relative bg-white flex flex-col justify-center p-8 sm:p-12 lg:p-24 border-l-4 border-black overflow-y-auto">
        ...
        <div className="absolute top-8 left-8 flex items-center gap-6 z-30">
          ...
          >
             <ArrowLeft className="w-5 h-5 text-black group-hover:text-[#2563EB]" />
          </button>
        ...
        <div className="max-w-md w-full mx-auto mt-24 lg:mt-0">
          <div className="mb-10 relative">
            <h2 className="text-6xl lg:text-7xl text-black uppercase mb-2 tracking-wide drop-shadow-[2px_2px_0px_#2563EB]">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {!isLogin && (
              <div className="mb-10">
                <label className="block text-black font-['Bangers'] text-xl uppercase tracking-wider mb-4 text-center border-b border-black/10 pb-2">
                  Choisissez votre rôle :
                </label>
                <div className="flex gap-4 mb-8">
                  <button type="button" onClick={() => setUserType('reader')} className={`flex-1 flex flex-col items-center gap-2 p-4 border-2 transform -skew-x-12 transition-all ${userType === 'reader' ? 'bg-black border-black text-white shadow-[4px_4px_0px_0px_#2563EB]' : 'bg-transparent border-black/40 text-black/60 hover:border-black hover:text-black'}`}>
                  ...
                  <button type="button" onClick={() => setUserType('author')} className={`flex-1 flex flex-col items-center gap-2 p-4 border-2 transform -skew-x-12 transition-all ${userType === 'author' ? 'bg-black border-black text-white shadow-[4px_4px_0px_0px_#2563EB]' : 'bg-transparent border-black/40 text-black/60 hover:border-black hover:text-black'}`}>
            ...
            <ParallelogramInput label="Email" placeholder="votre@alias.bd" value={email} onChange={(e) => setEmail(e.target.value)} error={errors.email} />
            <ParallelogramInput label="Mot de passe" type="password" placeholder="••••••••••••" value={password} onChange={(e) => setPassword(e.target.value)} error={errors.password} />
                ...
                <div className="space-y-3 bg-gray-50 p-4 border-2 border-black/20">
                     <label className="flex items-start gap-3 cursor-pointer">
                        ...
                        <span className="text-sm font-bold text-gray-700">
                            J'accepte les <button type="button" onClick={() => setShowLegal('cgu')} className="text-[#2563EB] underline hover:text-black">Conditions Générales (CGU)</button>.
                        </span>
                     </label>
                     <label className="flex items-start gap-3 cursor-pointer">
                        <input
                        ...
                        />
                        <span className="text-sm font-bold text-gray-700">
                            J'ai lu la <button type="button" onClick={() => setShowLegal('privacy')} className="text-[#2563EB] underline hover:text-black">Politique de Confidentialité</button>.
                        </span>
                     </label>
                     {errors.consent && <p className="text-[#EA4335] text-xs font-black uppercase">{errors.consent}</p>}
                </div>
            )}
            ...
            <div className="flex items-center justify-between py-2">
               {isLogin && (
                 <label className="flex items-center space-x-3 cursor-pointer group">
                    <div className="w-5 h-5 border-2 border-black bg-transparent flex items-center justify-center transition-all group-hover:border-[#2563EB]">
                    ...
                    <span className="font-['Bangers'] text-base uppercase tracking-wider text-black group-hover:text-[#2563EB] mt-0.5">Rester connecté</span>
                 </label>
               )}
               {isLogin && (
                 <a href="#" className="font-['Bangers'] text-base uppercase tracking-wider text-[#2563EB] hover:text-[#2563EB]/80 transition-colors mt-0.5">Mot de passe oublié ?</a>
               )}
            </div>
          ...
          <div className="flex justify-center">
             <button type="button" className="w-full relative h-14 transform -skew-x-12 border-2 border-black bg-white hover:bg-black/5 transition-all shadow-[6px_6px_0px_0px_#2563EB] active:shadow-none active:translate-x-[6px] active:translate-y-[6px]">
                <div className="transform skew-x-12 flex items-center justify-center gap-4 h-full w-full">
                  ...
                  <span className="font-['Bangers'] text-lg uppercase tracking-wide text-black mt-1">Google</span>
                </div>
             </button>
          </div>
        </div>
